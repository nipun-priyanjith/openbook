...
# a) Generate 1-D array of 8 random integers between 40 and 50
arr1 = np.random.randint(40, 51, 8)
print("Random integers:", arr1)
# Example output: Random integers: [44 50 46 48 41 43 44 48]

# b) Replace all odd numbers with -10
arr1[arr1 % 2 != 0] = -10
print("Odd numbers replaced:", arr1)
# Example output: Odd numbers replaced: [44 50 46 48 -10 -10 44 48]

# c) Extract first four columns from 2-D array
arrC = np.arange(100).reshape(5, 20)  # 5 rows, 20 cols
first_four_cols = arrC[:, :4]
...
# d) Clip values greater than 30 to 30, less than 10 to 10
np.random.seed(100)
arraD = np.random.uniform(1,50, 20)
arraD = np.clip(arraD, 10, 30)
print("Clipped array:", arraD)
# Example output: Clipped array: [26.869875 24.850019 ... 30.        10.       ]

# e) Swap rows 1 and 2
arrE = np.arange(16).reshape(4,4)
arrE[[1,2]] = arrE[[2,1]]
print("Swapped rows:\n", arrE)
# Example output:
# [[ 0  1  2  3]
...
# f) 4x4 array of letters 'a' to 'p'
letters = np.array(list('abcdefghijklmnop')).reshape(4,4)
print("Original letters array:\n", letters)
# [['a' 'b' 'c' 'd']
#  ['e' 'f' 'g' 'h']
#  ['i' 'j' 'k' 'l']
#  ['m' 'n' 'o' 'p']]

# Convert last row to uppercase
letters[-1] = np.char.upper(letters[-1])
print("Uppercase last row:\n", letters)
# Last row: ['M' 'N' 'O' 'P']

# Replace value at 3rd row, 2nd column with first name
letters[2,1] = 'Nipun'
print("Updated letters array:\n", letters)
# Row 3, col 2 now: 'Nipun'
...
matrix2 = np.array([[1,0],[2,1]])

print("Matrix1 symmetric?", is_symmetric(matrix1))  # True
print("Matrix2 symmetric?", is_symmetric(matrix2))  # False

# *********************************************
# 4. Structured Arrays (Employee Records)
# *********************************************
dtype_emp = [('Name', 'U10'), ('Age', 'i4'), ('Salary', 'f4')]
employees = np.array([
    ('Alice', 28, 48000.0),
    ('Bob', 34, 52000.0),
    ('Charlie', 25, 60000.0)
...
# [('Alice', 28, 48000.) ('Bob', 34, 52000.) ('Charlie', 25, 60000.)]

# Retrieve employees earning > 50,000
high_earners = employees[employees['Salary']>50000]['Name']
print("High earners:", high_earners)
# ['Bob' 'Charlie']

# *********************************************
# 5. Random Data Simulation & Statistics
# *********************************************
np.random.seed(32)
exam_scores = np.random.normal(loc=20, scale=6, size=250)

above_25 = np.sum(exam_scores>25)
between_15_50 = np.sum((exam_scores>=15) & (exam_scores<=50))
below_45 = np.sum(exam_scores<45)

print("Above 25:", above_25)       # e.g., 56
print("Between 15-50:", between_15_50)  # e.g., 234
print("Below 45:", below_45)       # e.g., 250

# *********************************************
# 6. Python Functions for Equations
# *********************************************
def f1(x, y, z):
    return math.sqrt(3) + y**2 + math.log(z+y)

print("f1(x=5,y=10,z=2):", f1(5,10,2))
# Example output: 102.098612

def f2(x,y,z):
    return math.exp(2/3) * math.sin(y) + math.sqrt(z**2 + y**2)

print("f2(x=2,y=11,z=3):", f2(2,11,3))
# Example output: 11.511
...
df.reset_index(drop=True, inplace=True)
df.rename(columns={'Age':'CustomerAge'}, inplace=True)

print("Cleaned DataFrame:\n", df)

# *********************************************
# 9. Sorting, Filtering & Grouping
# *********************************************
print("Sorted by CustomerAge:\n", df.sort_values('CustomerAge'))
print("Group by City mean Salary:\n", df.groupby('City')['Salary'].mean())
print("Pivot table (Salary by City & Name):\n", df.pivot_table(values='Salary', index='City', columns='Name', aggfunc='mean'))

# *********************************************
# 10. Pandas & NumPy Plotting
# *********************************************
df['CustomerAge'].plot(kind='line', title='Line Plot of Age')
plt.show()

df['Salary'].plot(kind='bar', title='Bar Plot of Salary')
plt.show()

df['Salary'].plot(kind='hist', bins=5, title='Histogram of Salary')
plt.show()

df.plot.scatter(x='CustomerAge', y='Salary', title='Scatter plot Age vs Salary')
plt.show()